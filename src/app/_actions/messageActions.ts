...
export async function requestToMessage(
  ...
  try {
    ...
    const status = newRequest.status as MessageRequestStatus;
...
export async function respondToMessageRequest(
  ...
  try {
    ...
    if (
      validated.response === MESSAGE_REQUEST_STATUS.ACCEPTED &&
    ...
    console.log(`✅ Request ${validated.response}:`, requestId);
  ...
  try {
    ...
    return {
      ...
      sourceRequestId: session.source_request_id
        ...
        : null,
      createdAt: timestampSchema.parse(new Date(session.created_at)), // ✅ Parse as branded timestamp
      status: messageSessionStatusSchema.parse(session.status), // ✅ Use correct status schema
...
export async function submitMessage(
  ...
  try {
    ...
    const message: Message = {
      ...
      sessionId,
      senderCheckinId,
      content: sanitizedContentSchema.parse(validated.content), // ✅ Parse as branded content
...
export async function getMessageRequests(
  ...
  try {
    ...
    return (data || []).map((req) => ({
      ...
      status: messageRequestStatusSchema.parse(req.status), // ✅ Parse through schema
      createdAt: timestampSchema.parse(new Date(req.created_at)), // ✅ Parse as branded timestamp
...
export async function getSessionMessages(
  ...
  try {
    ...
    return (data || []).map((msg) => ({
      ...
      content: sanitizedContentSchema.parse(msg.content), // ✅ Parse content
      createdAt: timestampSchema.parse(new Date(msg.created_at)), // ✅ Parse timestamp
      deliveredAt: msg.delivered_at
        ? timestampSchema.parse(new Date(msg.delivered_at))
        : undefined, // ✅ Use undefined for optional fields
      readAt: msg.read_at
